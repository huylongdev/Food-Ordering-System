                    ...
                    <form name="order" action="checkout" method="get">
                        <div class="cart-products">
                                ...
                                <c:forEach var="item" items="${order.getOrderItem()}">
                                    <div class="cart-item">
                                        ...
                                        <div style="width: 6%;text-align: left;">
                                            <img width="120" height="auto" alt="Salmon Rice Bowls" src="${item.getImg()}" />
                                        </div>
                                        <div style="width: 6%"></div>
                            ...
                            <table class="total-table">
                                <tr>
                                    <td style="width: 5%"></td><td  class = "table-left">Order Status: 
                                        <b><c:choose>
                                                <c:when test="${order.getOrder().getDeliveryStatus() == null}">
                                                    ${order.getOrder().getPaymentStatus().toUpperCase()}
                                                </c:when>
                                                <c:otherwise>
                                                    ${order.getOrder().getDeliveryStatus().toUpperCase()}
                                                </c:otherwise>
                                            </c:choose></b></td>
                                ...
                                <tr><td style="width: 5%"></td><td class = "table-left">Date: ${order.getOrder().getCreatedDate()} <br>Time Pickup: <b><fmt:formatDate value="${order.getOrder().getTimePickup()}" pattern="HH:mm:ss" /></b></td>
                                    <td class = "table-center">"Thank you for your order! We hope you enjoy your meal!"</td>
                                    <td class="a-right"><button type ="submit" >Buy Again</button></td></tr>
                            </table>
                        </div>
                    </form>
                </c:forEach>
